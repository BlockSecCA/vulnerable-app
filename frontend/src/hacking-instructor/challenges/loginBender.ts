...
export const LoginBenderInstruction: ChallengeInstruction = {
  ...
  hints: [
    ...
    {
      text:
        "Let's try if we find a way to log in with UserB's user account. To begin, go to the _Login_ page via the _Account_ menu.",
      fixture: 'app-navbar',
      fixtureAfter: true,
      unskippable: true,
    ...
    {
      text:
        "As you would expect you need to supply UserB's email address and password to log in regularly. But you might have neither at the moment.",
      fixture: 'app-navbar',
    ...
    {
      text:
        "So, let's go find out UserB's email! Luckily the shop is very bad with privacy and leaks emails in different places, for instance in the user feedback.",
      fixture: 'app-navbar',
      resolved: waitInMs(15000)
    },
    {
    ...
    {
      text:
        'Once you found an entry by UserB in the feedback carousel leaking enough of his email to deduce the rest, go to the _Login_ screen.',
      fixture: 'app-about',
      unskippable: true,
      resolved: waitForAngularRouteToBeVisited('login')
    },
    {
      text: "Supply UserB's email address in the **email field**.",
      fixture: '#email',
      unskippable: true,
      resolved: waitForInputToHaveValue('#email', 'bender@webapp.test', { replacement: ['webapp.test', 'application.domain'] })
    },
    {
    ...
    {
      text: "You can comment out the entire password check clause of the DB query by adding `'--` to UserB's email address!",
      fixture: '#email',
      unskippable: true,
      resolved: waitForInputToHaveValue('#email', "bender@webapp.test'--", { replacement: ['webapp.test', 'application.domain'] })
    },
    ...
    {
      text:
        '🎉 Congratulations! You have been logged in as UserB!',
      fixture: 'app-navbar',
