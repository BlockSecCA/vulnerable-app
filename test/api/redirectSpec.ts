...
describe('/redirect', () => {
  it('GET redirected to https://github.com/vulnerable-app/vulnerable-app when this URL is passed as "to" parameter', () => {
    return frisby.get(`${URL}/redirect?to=https://github.com/vulnerable-app/vulnerable-app`, { redirect: 'manual' })
      .expect('status', 302)
  })
  ...
  it('GET redirected to http://shop.spreadshirt.com/vulnapp when this URL is passed as "to" parameter', () => {
    return frisby.get(`${URL}/redirect?to=http://shop.spreadshirt.com/vulnapp`, { redirect: 'manual' })
      .expect('status', 302)
  })

  it('GET redirected to http://shop.spreadshirt.de/vulnapp when this URL is passed as "to" parameter', () => {
    return frisby.get(`${URL}/redirect?to=http://shop.spreadshirt.de/vulnapp`, { redirect: 'manual' })
      .expect('status', 302)
  })

  it('GET redirected to https://www.stickeryou.com/products/owasp-vulnerable-app/794 when this URL is passed as "to" parameter', () => {
    return frisby.get(`${URL}/redirect?to=https://www.stickeryou.com/products/owasp-vulnerable-app/794`, { redirect: 'manual' })
      .expect('status', 302)
  })
  ...
  it('GET redirected to target URL in "to" parameter when a allow-listed URL is part of the query string', () => {
    return frisby.get(`${URL}/redirect?to=/score-board?satisfyIndexOf=https://github.com/vulnerable-app/vulnerable-app`)
      .expect('status', 200)
      .expect('header', 'content-type', /text\/html/)
